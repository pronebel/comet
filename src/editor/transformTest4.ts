...
const { red, green, blue, selection, white, win } = setup();
...
function drawBorder(matrix: Matrix)
{
    const width = 16;
    const height = 16;
    const p1 = matrix.apply({ x: 0, y: 0 });
    ...
    selection.lineStyle(1, 0x00ffff, 1);
    selection.moveTo(p1.x, p1.y); selection.lineTo(p2.x, p2.y);
    selection.moveTo(p2.x, p2.y); selection.lineTo(p3.x, p3.y);
    selection.moveTo(p3.x, p3.y); selection.lineTo(p4.x, p4.y);
    selection.moveTo(p4.x, p4.y); selection.lineTo(p1.x, p1.y);
...
white.scale.set(2, 1);
white.angle = 45;
white.updateTransform();

// const transform = new Transform();

// transform.position.set(50, 0);
// transform.scale.set(2, 1);
// transform.rotation = degToRad(45);
// transform.updateLocalTransform();
...
transformMatrix.rotate(degToRad(rotation));
transformMatrix.translate(x, y);

// const transformMatrix = transform.localTransform;
const viewMatrix = white.worldTransform;
const matrix = new Matrix();

// matrix.append(viewMatrix);
// matrix.translate(-viewMatrix.tx, -viewMatrix.ty);
// matrix.append(transformMatrix);
// matrix.translate(viewMatrix.tx, viewMatrix.ty);

matrix.append(transformMatrix);
matrix.translate(-transformMatrix.tx, -transformMatrix.ty);
matrix.append(viewMatrix);
matrix.translate(transformMatrix.tx, transformMatrix.ty);
